...
pub mod staking_pool_rewards {
    ...
    pub fn deposit_rewards(ctx: Context<DepositRewards>, amount: u64) -> Result<()> {
        ...
        require!(ctx.accounts.pool.total_staked > 0, PoolError::NoActiveStakers);
    ...
    pub fn unstake(ctx: Context<Unstake>, amount: u64) -> Result<()> {
        ...
        require_keys_eq!(
            ctx.accounts.user_stake_token.owner,
            ctx.accounts.user.key(),
            PoolError::InvalidStakeDestination
        );
    ...
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        ...
        require_keys_eq!(
            ctx.accounts.user_reward_token.owner,
            ctx.accounts.user.key(),
            PoolError::InvalidRewardDestination
        );
...
fn initialize_or_validate_position(
    ...
    validate_position_for_owner(position, pool, owner)
}

fn validate_position_for_owner(position: &Position, pool: Pubkey, owner: Pubkey) -> Result<()> {
    require_keys_eq!(position.pool, pool, PoolError::InvalidPositionPool);
    require_keys_eq!(position.owner, owner, PoolError::InvalidPositionOwner);
    Ok(())
}
...
pub enum PoolError {
    ...
    #[msg("Invalid position pool")]
    InvalidPositionPool,
    #[msg("Invalid stake destination")]
    InvalidStakeDestination,
    #[msg("Invalid reward destination")]
