...
pub mod vault_receipt_migration {
    ...
    pub fn migrate_vault(ctx: Context<MigrateVault>) -> Result<()> {
        ...
        require_keys_eq!(
            vault_config.admin,
            ctx.accounts.admin.key(),
            VaultMigrationError::UnauthorizedAdmin
        );
        ...
        require!(
            vault_config.version == LEGACY_VERSION,
            VaultMigrationError::InvalidVersion
        );
    ...
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        ...
        require!(
            receipt.version == CURRENT_VERSION,
            VaultMigrationError::ReceiptNeedsMigration
        );
        require!(
        ...
        require_keys_eq!(
            ctx.accounts.user_destination_token.owner,
            ctx.accounts.user.key(),
            VaultMigrationError::InvalidTokenAccount
        );
        require_keys_eq!(
            ctx.accounts.user_destination_token.mint,
            ctx.accounts.mint.key(),
            VaultMigrationError::InvalidTokenAccount
        );
