...
use vault_receipt_migration::{
    accounts, instruction, Receipt, VaultConfig, ID,
};
...
pub fn initialize_legacy_vault(context: &TestContext) -> TestResult<Signature> {
    Ok(context
        .program
        .request()
        .accounts(accounts::InitializeLegacyVault {
            admin: context.admin,
        ...
        .args(instruction::InitializeLegacyVault { seed: context.seed })
        .send()?)
}
...
pub fn initialize_vault(context: &TestContext) -> TestResult<Signature> {
    Ok(context
        ...
        .args(instruction::InitializeVault { seed: context.seed })
        .send()?)
}

pub fn migrate_vault(context: &TestContext) -> TestResult<Signature> {
    migrate_vault_as(context, context.admin, None)
}

pub fn migrate_vault_as(
    context: &TestContext,
    admin: Pubkey,
...
pub fn migrate_receipt_default(context: &TestContext) -> TestResult<Signature> {
    migrate_receipt(
        context,
        context.user.pubkey(),
        &context.user,
...
pub fn migrate_receipt(
    ...
    Ok(context
        .program
        .request()
        .accounts(accounts::MigrateReceipt {
            user,
            vault_config: context.vault_config,
            receipt,
        })
        .args(instruction::MigrateReceipt {})
        .signer(signer)
        .send()?)
}
