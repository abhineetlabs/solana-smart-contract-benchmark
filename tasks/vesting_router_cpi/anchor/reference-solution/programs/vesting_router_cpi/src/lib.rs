...
pub mod vesting_router_cpi {
    ...
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        ...
        require_keys_eq!(
            ctx.accounts.stream.beneficiary,
            ctx.accounts.beneficiary.key(),
            StreamError::InvalidBeneficiary
        );
        require_keys_eq!(
            ctx.accounts.stream.vault,
            ctx.accounts.vault.key(),
            StreamError::InvalidVault
        );
        require_keys_eq!(
            ctx.accounts.vault.controller,
            ctx.accounts.router_authority.key(),
            StreamError::InvalidVaultController
        );
        require_keys_eq!(
            ctx.accounts.beneficiary_token.owner,
            ctx.accounts.beneficiary.key(),
            StreamError::InvalidRecipientToken
        );

        let vested_total = vested_amount(&ctx.accounts.stream)?;
        let claim_amount = vested_total
            .checked_sub(ctx.accounts.stream.claimed_amount)
...
pub struct Fund<'info> {
    ...
    #[account(
        ...
        constraint = stream.admin == admin.key() @ StreamError::UnauthorizedAdmin
    )]
    pub stream: Account<'info, Stream>,
    pub mint: Account<'info, Mint>,
    #[account(
...
pub struct Claim<'info> {
    ...
    #[account(
        mut,
        constraint = stream.vault == vault.key() @ StreamError::InvalidVault
    )]
...
pub enum StreamError {
    ...
    #[msg("Invalid vault")]
    InvalidVault,
    #[msg("Invalid vault controller")]
    InvalidVaultController,
    #[msg("Invalid recipient token")]
