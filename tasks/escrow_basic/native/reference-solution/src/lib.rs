...
fn initialize(
    ...
    if offered_amount == 0 || desired_amount == 0 {
        return Err(EscrowError::AmountMustBePositive.into());
    }
...
fn exchange(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    if escrow.maker != *maker_info.key {
        return Err(EscrowError::InvalidMaker.into());
    }
    if escrow.offered_mint != *offered_mint_info.key || escrow.desired_mint != *desired_mint_info.key
    {
    ...
    let maker_receive_token = load_token_account(maker_receive_token_info)?;
    if maker_receive_token.owner != *maker_info.key
        || maker_receive_token.mint != *desired_mint_info.key
    {
        return Err(EscrowError::InvalidMakerTokenAccount.into());
    }
    ...
    let taker_receive_token = load_token_account(taker_receive_token_info)?;
    if taker_receive_token.owner != *taker_info.key
        || taker_receive_token.mint != *offered_mint_info.key
    {
        return Err(EscrowError::InvalidTakerTokenAccount.into());
    }
...
fn cancel(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let maker_receive_token = load_token_account(maker_receive_token_info)?;
    if maker_receive_token.owner != *maker_info.key
        || maker_receive_token.mint != *offered_mint_info.key
    {
        return Err(EscrowError::InvalidMakerTokenAccount.into());
    }
